...
- Forgot password (Super Admin login page → "Password bhool gaye?"): (1) OTP on the recovery mobile via the MSG91 widget — the person types the number, it must match admin.recoveryPhone (default 9389585479); (2) one-time recovery code made in Site Settings → 🔐 Recovery (stored hashed, single use). Both rate-limited and logged in Activity Log. Wrong "default login" hint removed from the login page.
...
- All panels (Super Admin, Sub-Admin, Technician): header / sidebar in very light grey (#f1f3f5) with dark text, active item white with orange marker; technician top and bottom bars grey too.
- Main customer site header now uses the same very light grey (#f1f3f5) as the panels; logo blended into it, darker text for contrast.
