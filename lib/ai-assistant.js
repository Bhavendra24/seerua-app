...
${knownCustomer ? `
🔴 RETURNING CUSTOMER DETECTED — a phone number in this conversation matches someone who has booked with Seerua before. ${knownCustomer.address
  ? `This chat is on the customer's OWN phone (verified by the site). Their details on file: name "${knownCustomer.name}", address "${knownCustomer.address}"${knownCustomer.cityName ? `, city "${knownCustomer.cityName}"` : ''}. The chat window itself shows them this address with a "Yes, same address" button — don't repeat the address, just greet them warmly by first name and carry on; if they confirm it, don't ask for name/address again.`
  : `Their details on file: ${knownCustomer.name ? `first name "${String(knownCustomer.name).trim().split(/\s+/)[0]}"` : ''}${knownCustomer.cityName ? `, city "${knownCustomer.cityName}"` : ''}. 🔴 PRIVACY: this is NOT verified as their own phone, so NEVER reveal or hint at a stored address or full name — only greet them by first name, and ask for the full address as normal.`} Since the phone number is now the very first thing you ask for in a booking (see the numbered list below), this should come up early — right after they give it, before you'd otherwise ask for their name/address at all. Give a warm "welcome back" using their first name.
` : ''}
${customInstructions ? `
...
4. Their full name
5. Full address (house/flat number, area/locality; ask for a landmark only if they don't give enough to find the place) — skip only if a returning customer on their own phone confirmed their saved address
