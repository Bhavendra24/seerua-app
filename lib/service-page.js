...
// Same visual "was" price the rest of the site already uses (homepage
// Quick Book + old cards) — kept identical so the two never disagree.
function mrpFor(price) {
...
function contentFor(appliance) {
  const key = slugify(appliance.name);
  const name = appliance.name;
  return APPLIANCE_CONTENT[key] || {
    problems: [
      [`${name} not working at all`, 'power supply, wiring or control board faults.'],
      ['Unusual noise or smell', 'a worn part or loose component that should be checked early.'],
      ['Poor performance', 'dirt buildup or a part nearing the end of its life.'],
      ['Leakage', 'loose connections or a worn seal.']
    ],
    tips: [
      ['Keep it clean', 'regular cleaning prevents most everyday problems.'],
      ['Book a routine service', 'small issues are cheaper to fix early.'],
      ['Use a stable power supply', 'voltage swings damage electronics.']
    ],
      ...
      return `
        <article class="sp-card" ${data}>
          ...
          <div class="sp-card-body">
            ...
            <div class="sp-price">${TAG_SVG}<s>${inr(mrpFor(price))}</s><strong>${inr(price)}</strong></div>
...
function buildArticleHtml({ appliance, city, typeRows, focusType, displayName, phoneDisplay, serviceProcessHtml, aboutText }) {
  ...
  const typeSections = (focusType ? typeRows.filter(r => r.type.id === focusType.id) : typeRows).map(({ type, services }) => {
    ...
    const svcList = services.map(({ svc, price }) => `<li><strong>${tn} ${esc(svc.name)}</strong> — from ${inr(price)}</li>`).join('');
    return `
