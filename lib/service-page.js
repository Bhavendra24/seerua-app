...
// Optional "regular" price Admin can set per service (Pricing tab) to run
// an offer: shown crossed out next to the real price, only when it's
// actually higher than what the customer pays.
function mrpForSku(row, svcId, price) {
  const m = row && row.mrpPrices ? Number(row.mrpPrices[svcId]) : 0;
...
function resolveTypeRows(appliance, city, pricing) {
  return appliance.types.map(t => {
    ...
    const services = servicesOf(t)
      .map(svc => { const price = priceForSku(row, svc.id); return { svc, price, mrp: mrpForSku(row, svc.id, price) }; })
...
function buildPanelsHtml(typeRows, appliance, city, activeTypeId, photoUrlFor) {
  return typeRows.map(({ type, services }) => {
    ...
    const cards = services.map(({ svc, price, mrp }) => {
      ...
      return `
        <article class="sp-card" ${data}>
          ...
          <div class="sp-card-body">
            ...
            <div class="sp-price">${TAG_SVG}${mrp ? `<s class="sp-mrp">${inr(mrp)}</s>` : ''}<strong>${inr(price)}</strong>${mrp ? `<span class="sp-off">${inr(mrp - price)} off</span>` : ''}${needsPartsNote(svc) ? '<span class="sp-parts">+ parts, if needed (with your OK)</span>' : ''}</div>
